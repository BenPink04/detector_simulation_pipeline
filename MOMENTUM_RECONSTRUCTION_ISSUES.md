...
2. [Issue 1: Systematic Momentum Underestimation](#2-issue-1-systematic-momentum-underestimation)
   ...
   - 2.5 First Attempted Fix (track extrapolation — worsened bias)
   - 2.6 Correct Fix: Bar-ID Smeared Position
...
The initial fix replaced the bar-centre+smear positions with track-extrapolated positions, and added the near-parallel cut:
...
// (C) PIZZA position from track extrapolation — CORRECT
TVector3 pip_pizza_pos = eval_track_at_z(fit_pip, pip_pizza_z);
TVector3 pim_pizza_pos = eval_track_at_z(fit_pim, pim_pizza_z);

// (D) TOF position from track extrapolation — STILL INCORRECT (see §2.6)
TVector3 pip_tof_pos = eval_track_at_z(fit_pip, z_tof);
TVector3 pim_tof_pos = eval_track_at_z(fit_pim, z_tof);
```

The PIZZA fix (C) is correct because the track fit is constrained by tracker and FRI hits and is not anchored to the PIZZA bar geometry. However, this fix made the momentum underestimation **worse** for the TOF position (D), for the reason explained in §2.6.

### 2.6 Correct Fix: Bar-ID Smeared Position

**Why `eval_track_at_z` fails for the TOF position:**

The `fit_3dgraph_track` function adds the TOF hit to both the x(z) and y(z) fits using the **bar-centre geometry**:

```cpp
xv.push_back(tof_bar_x_centre(tof_devID));   // bar centre X — NOT the true hit X
yv.push_back(tof_bar_y_centre(tof_devID));   // bar centre Y — NOT the true hit Y
```

The fitted track direction is therefore pulled toward the bar centre at `z_tof`. Calling `eval_track_at_z(fit_pip, z_tof)` then returns a position very close to the bar centre — essentially the same as the original bar-centre approach, but now without any random smear to occasionally partially cancel the bias. The result is a systematic bar-centre bias that is **more consistent but more wrong** than random bar-centre + Gaus.

The root constraint is physical: the time on `pip_tof_time` is the true Geant4 hit time at the actual particle impact point, but the track fit is anchored to the bar centre. These can never be made consistent by extrapolating the same track that was anchored to bar centre.

**The correct approach:** compute the TOF position estimate independently from the track fit, using only what the detector geometry actually measures:

- **x**: which bar was hit is known precisely → `x ~ Gaus(bar_centre_x, TOF_BAR_HALF_WIDTH)` (6 cm half-width)
- **y**: measured by timing along the bar → `y ~ Gaus(bar_centre_y, TOF_Y_UNCERTAINTY)` (10 cm sigma)
- **z**: the bar face position from the geometry tag → `z = z_tof` (known precisely)

These smear magnitudes are the same uncertainties already used in the track fit, so no new parameters are introduced. Crucially, the smear is applied **at hit-collection time** (stored in `EventReco`), independently of the time smear, so the time and position are now two independent estimates of the same physical event — physically correct.

```cpp
// In hit-collection loop (stored once in EventReco):
...
// In velocity calculation (replaces eval_track_at_z for TOF):
TVector3 pip_tof_pos(pip_tof_x, pip_tof_y, pip_tof_z);
TVector3 pim_tof_pos(pim_tof_x, pim_tof_y, pim_tof_z);
```

The track fit (`fit_3dgraph_track`) still includes the TOF bar point as before, which helps constrain the track direction for the PoCA vertex. Only the **path-length calculation** is switched to the independently-smeared bar-ID position.
...
| 5 | Both macros, TOF position computation | Replace `bar_centre + Gaus(0,10)` / `eval_track_at_z` with bar-ID smeared position stored in `EventReco` at hit-collection time | Position and time are independent measurements of the same hit; `eval_track_at_z` returns bar centre (track anchored there) and worsens bias — see §2.6 |

All 5 changes have been applied to both `KLong_save_vectors.C` and `KLong_save_momentum_acceptance.C`. ✅

> **Note on Change 5 history**: an intermediate version used `eval_track_at_z(fit_pip, z_tof)` for the TOF position, which made the underestimation worse. See §2.6 for the full explanation.
...
// Change 5: TOF position from bar-ID smear (stored in EventReco at hit-collection time)
//   x ~ Gaus(bar_centre_x, TOF_BAR_HALF_WIDTH=6cm)   — which bar fired
//   y ~ Gaus(bar_centre_y, TOF_Y_UNCERTAINTY=10cm)   — from light-path timing along bar
//   z = z_tof from geometry tag                       — known precisely
// Applied once when the earliest TOF hit is recorded; stored in ev.pip_tof_x/y/z.
// Using eval_track_at_z for TOF is incorrect: the track fit is anchored to bar centre
// via tof_bar_x_centre(), so eval_track_at_z returns bar centre — see §2.6.
TVector3 pip_tof_pos(pip_tof_x, pip_tof_y, pip_tof_z);
TVector3 pim_tof_pos(pim_tof_x, pim_tof_y, pim_tof_z);
// Time and position are now two independent smeared estimates of the same physical hit
```
...
*Document created March 2026. Changes 1–4 applied March 17, 2026. Change 5 (TOF position) revised March 18, 2026 after track-extrapolation approach was found to worsen bias (see §2.6).*
