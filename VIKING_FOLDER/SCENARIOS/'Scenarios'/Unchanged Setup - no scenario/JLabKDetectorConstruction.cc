...
G4VPhysicalVolume* JLabKDetectorConstruction::Construct()
{
  ...
  G4double dipExtrHalfXY = dipHalfEdgeL - dipThk;
  ...
  new G4PVPlacement(0, G4ThreeVector(0., 0., dipPosZ), dipole_log, "Dipole",
                    expHall_log, false, 1, checkOverlaps);
  //------------------------------------------------------------------------  
  ...
  G4double strawPosX[122] = {-48.4*cm, -47.6*cm, -46.8*cm, -46.0*cm, -45.2*cm, -44.4*cm, -43.6*cm, -42.8*cm,
			     ...
			     -22.8*cm, -22.0*cm, -21.2*cm, -20.4*cm, -20.4*cm, -19.6*cm, -18.8*cm, -18.0*cm,
  ...
  G4LogicalVolume* trkStraw_log = new G4LogicalVolume(trkStraw, aluminium, 
                                                    "StrawTube");
  ...
  G4double trkrPhi[4] = {45.*deg, 0.*deg, -45.*deg, -90.*deg};
  //G4double trkrPosZ[4] = {350.*cm, 380.*cm, 410.*cm, 440.*cm};
  G4double trkrPosZ[4] = {300.*cm, 350.*cm, 1300.*cm, 1350.*cm};
  ...
  for (G4int i = 0; i < 4; i++)
  {
    ...
    new G4PVPlacement(trkrRot, G4ThreeVector(0., 0., trkrPosZ[i]), trkrBox_log,
                      "Tracker", expHall_log, false, 2 + i, checkOverlaps);
  }
  //------------------------------------------------------------------------  
  ...
  G4LogicalVolume* pizzaSlice_log = new G4LogicalVolume(pizzaSlice, aluminium, 
                                                        "PizzaSlice");
  ...
  for (G4int i = 0; i < 24; i++)   //Modify for number of slices in WASA detector (24)
  {
    ...
    new G4PVPlacement(pizzaSliceRot, G4ThreeVector(0., 0., 0.), pizzaSlice_log,
                      "PizzaSlice", pizza1_log, false, i, checkOverlaps);
  }
  ...
  for (G4int i = 0; i < 24; i++)   //Modify for number of slices in WASA detector (24)
    {
      ...
      new G4PVPlacement(pizzaSliceRot, G4ThreeVector(0., 0., 0.), pizzaSlice_log,
			"PizzaSlice", pizza2_log, false, i, checkOverlaps);
    }
  ...
  G4double pizzaPosZ[4] = {500.*cm, 525.*cm, 1100.*cm, 1200.*cm};
  ...
  G4VSolid* friWallWithHole = new G4SubtractionSolid(
        ...
        0,                      // Rotation (N/A so 0)
        G4ThreeVector(0., 0., 0.)  // Position - center (LOOK TO ADJUST)
  );

  //Replace original FriWall with new solid with subtracted center
  G4LogicalVolume* friWall_log = new G4LogicalVolume(
        friWallWithHole,        // now using subtracted solid
        air,                    // material change later
        "FriWall"
  );
  ...
  G4LogicalVolume* friStrip_log = new G4LogicalVolume(friStrip, aluminium, 
                                                        "FriStrip");
  ...
  G4LogicalVolume* friStripLong_log = new G4LogicalVolume(friStripLong, aluminium, 
                                                        "FriStripLong");
  ...
  G4LogicalVolume* friStrip_1_log = new G4LogicalVolume(friStrip_1, aluminium, 
                                                        "FriStrip1");
  ...
  G4LogicalVolume* friStrip_2_log = new G4LogicalVolume(friStrip_2, aluminium, 
                                                        "FriStrip2");
  ...
  G4LogicalVolume* friStrip_3_log = new G4LogicalVolume(friStrip_3, aluminium, 
                                                        "FriStrip3");
  ...
  G4LogicalVolume* friStrip_4_log = new G4LogicalVolume(friStrip_4, aluminium, 
                                                        "FriStrip4");
  ...
  G4LogicalVolume* friStripInner_log = new G4LogicalVolume(friStripInner, aluminium, 
                                                        "FriStripInner");
  ...
  G4LogicalVolume* friStripInner_1_log = new G4LogicalVolume(friStripInner_1, aluminium, 
                                                        "FriStripInner1");
  ...
  G4LogicalVolume* friStripInner_1_cut_log = new G4LogicalVolume(
        ...
        aluminium,                      // material change later
        "FriStripInner1_cut"
  );
  ...
  G4LogicalVolume* friStripInner_2_log = new G4LogicalVolume(friStripInner_2, aluminium, 
                                                        "FriStripInner2");
  ...
  G4LogicalVolume* friStripInner_3_log = new G4LogicalVolume(friStripInner_3, aluminium, 
                                                        "FriStripInner3");
  ...
  G4LogicalVolume* friStripInner_4_log = new G4LogicalVolume(friStripInner_4, aluminium, 
                                                        "FriStripInner4");
  ...
  G4double stripPosX[26] = {-66.*cm, -60.*cm, -54.*cm, -48.*cm, -42.*cm, -36.*cm, -30.*cm, -24.*cm, -18.*cm, -12.*cm, -7.5*cm,
  ...
  for (G4int i = 0; i < 26; i++)   //Modify for number of slices in WASA detector
  {
    if((i == 0) || (i == 25)){
      new G4PVPlacement(0, G4ThreeVector(stripPosX[i], 0., 0.), friStrip_log, "FriStrip",
			friWall_log, false, i, checkOverlaps);
    }
    else if((i == 1) || (i == 24)){
  ...
  G4double friPosZ[2] = {1100.*cm, 1125.*cm};  //replaces downstream pizzas from conceptual design
  //G4double friPosZ[2] = {500.*cm, 525.*cm};  //replaces downstream pizzas from conceptual design
  G4double friPhi[2] = {0.*deg, 90.*deg};
    ...
    for (G4int i = 0; i < 2; i++)
  {
     ...
     new G4PVPlacement(rot3, G4ThreeVector(0., 0., friPosZ[i]), friWall_log,
                       "FriWall", expHall_log, false, 8 + i, checkOverlaps);
  }
  ...
  G4LogicalVolume* tofBar_log = new G4LogicalVolume(tofBar, aluminium, 
                                                        "TofBar");

  G4VSolid* tofBarShort = new G4Box("TofBarShort",
			       tBarHalfX, tBarShortHalfY, tBarHalfZ);

  G4LogicalVolume* tofBarShort_log = new G4LogicalVolume(tofBarShort, aluminium, 
                                                        "TofBarShort");

  G4VSolid* tofBarShortUpper = new G4Box("TofBarShortUpper",
			       tBarHalfX, tBarShortUpperHalfY, tBarHalfZ);

  G4LogicalVolume* tofBarShortUpper_log = new G4LogicalVolume(tofBarShortUpper, aluminium, 
							      "TofBarShortUpper");
  ...
  for (G4int i = 0; i < 12; i++)   //Modify for number of bars used from WASA detector
  {
    if(i == 5){
      ...
      new G4PVPlacement(0, G4ThreeVector(barPosX[i], -72.*cm, 0.), tofBarShort_log, "TofBarShort",
			tofWall_log, false, i, checkOverlaps);
    }
    else if(i == 6){
      //upper TOF bar
  ...
  G4double tofPosZ = 1400.*cm;  //replace downstream pizzas

  new G4PVPlacement(0, G4ThreeVector(0., 0., tofPosZ), tofWall_log,
		    "TofWall", expHall_log, false, 10, checkOverlaps);
  //------------------------------------------------------------------------
