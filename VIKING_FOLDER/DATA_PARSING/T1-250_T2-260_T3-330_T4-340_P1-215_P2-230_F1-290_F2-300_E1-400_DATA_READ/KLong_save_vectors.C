...
// << " | True p: " << true_p_mag
    ...
    std::vector<TVector3> *v_true = &true_vertex;

    outTree->Branch("reco_p", &p_reco);
    outTree->Branch("true_p", &p_true);
    outTree->Branch("reco_vertex", &v_reco);
    outTree->Branch("true_vertex", &v_true);
    ...
    std::cout << "Saved kaon momentum and vertex vectors to kaon_vectors.root\n";
}