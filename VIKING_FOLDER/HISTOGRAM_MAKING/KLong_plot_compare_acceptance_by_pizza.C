...
void KLong_plot_compare_acceptance_by_pizza() {
    // Create output directory
    std::string output_dir = "acceptance_by_pizza";
    ...
    std::cout << "Found " << filenames.size() << " combined acceptance files across all directories" << std::endl;
    ...
    for (const auto& pizza_entry : pizza_groups) {
        ...
        for (size_t file_idx = 0; file_idx < files_labels.size(); ++file_idx) {
            ...
            std::string h_title = "Reconstruction Acceptance Comparison (" + pc.toString() + ")";
        ...
        std::string plot_title = "Kaon Reconstruction Acceptance Comparison (" + pc.toString() + ")";
        h_acceptance_vec[0]->SetTitle(plot_title.c_str());
        ...
        std::string output_filename = output_dir + "/KLong_acceptance_comparison_" + pc.toString() + ".png";
        c1->SaveAs(output_filename.c_str());
        
        std::cout << "\nComparison plot saved as: " << output_filename << std::endl;
        
        // Print summary statistics
        std::cout << "\nSummary Statistics for " << pc.toString() << ":" << std::endl;
