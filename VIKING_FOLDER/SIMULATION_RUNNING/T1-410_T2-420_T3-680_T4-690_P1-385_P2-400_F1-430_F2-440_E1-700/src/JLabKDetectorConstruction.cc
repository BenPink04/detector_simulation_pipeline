...
G4VSolid* apert = new G4Tubs("Aperture", 0., gdTube_outerRad,    // ← gdTube_outerRad undefined after commenting out GuideTube
  ...
  */
  ...
  //G4double trkrPosZ[4] = {410.*cm, 420.*cm, 680.*cm, 690.*cm}; // OLD - too close to dipole
  G4double trkrPosZ[4] = {410.*cm, 420.*cm, 680.*cm, 690.*cm}; // CORRECTED - proper positions
  ...
  //G4double pizzaPosZ[4] = {385.*cm, 400.*cm, 1100.*cm, 1200.*cm};
  G4double pizzaPosZ[4] = {385.*cm, 400.*cm, 1100.*cm, 1200.*cm};
  ...
  G4double friPosZ[2] = {430.*cm, 440.*cm};  //replaces downstream pizzas from conceptual design - Position of FRI 
  //G4double friPosZ[2] = {430.*cm, 440.*cm};  //OLD - too close to dipole
