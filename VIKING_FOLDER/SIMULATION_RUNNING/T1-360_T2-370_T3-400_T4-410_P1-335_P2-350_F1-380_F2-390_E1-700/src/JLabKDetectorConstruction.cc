...
G4VSolid* apert = new G4Tubs("Aperture", 0., gdTube_outerRad,    // ← gdTube_outerRad undefined after commenting out GuideTube
  ...
  */
  ...
  //G4double trkrPosZ[4] = {360.*cm, 370.*cm, 400.*cm, 410.*cm}; // OLD - too close to dipole
  G4double trkrPosZ[4] = {360.*cm, 370.*cm, 400.*cm, 410.*cm}; // CORRECTED - proper positions

  for (G4int i = 0; i < 4; i++)
  {
    // Only place tracker if position > 0 (0 means disabled)
    if (trkrPosZ[i] > 0.*cm)
    {
  ...
  for (G4int i = 0; i < 24; i++)   //Modify for number of slices in WASA detector (24)
    {
      ...
      G4RotationMatrix* pizzaSliceRot = new G4RotationMatrix;
      pizzaSliceRot->rotateZ(pizzaSlicePhi);
      pizzaSliceRot->rotateY(20.*deg);  // TO MAKE INVERTED: add negative angle flips cone direction
  ...
  //G4double pizzaPosZ[4] = {335.*cm, 350.*cm, 1100.*cm, 1200.*cm};
  G4double pizzaPosZ[4] = {335.*cm, 350.*cm, 1100.*cm, 1200.*cm};
  ...
  // {
    ...
    // Only place Pizza1 if position > 0 (0 means disabled)
    if (pizzaPosZ[1] > 0.*cm)
    {
    ...
    // Only place Pizza2 if position > 0 (0 means disabled)
    if (pizzaPosZ[0] > 0.*cm)
    {
  ...
  G4double friPosZ[2] = {380.*cm, 390.*cm};  //replaces downstream pizzas from conceptual design - Position of FRI 
  //G4double friPosZ[2] = {380.*cm, 390.*cm};  //OLD - too close to dipole
  ...
  friWall_log[1] = friWall2_log; // Second wall
    ...
    for (G4int i = 0; i < 2; i++)
  {
     // Only place FRI wall if position > 0 (0 means disabled)
     if (friPosZ[i] > 0.*cm)
     {
       G4RotationMatrix* rot3 = new G4RotationMatrix;
