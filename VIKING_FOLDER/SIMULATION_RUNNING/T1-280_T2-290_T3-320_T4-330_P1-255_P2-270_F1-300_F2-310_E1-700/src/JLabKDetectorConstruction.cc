...
G4VSolid* apert = new G4Tubs("Aperture", 0., gdTube_outerRad,    // ← gdTube_outerRad undefined after commenting out GuideTube
  ...
  */
  ...
  //G4double trkrPosZ[4] = {280.*cm, 290.*cm, 320.*cm, 330.*cm}; // OLD - too close to dipole
  G4double trkrPosZ[4] = {280.*cm, 290.*cm, 320.*cm, 330.*cm}; // CORRECTED - proper positions
  ...
  //G4double pizzaPosZ[4] = {255.*cm, 270.*cm, 1100.*cm, 1200.*cm};
  G4double pizzaPosZ[4] = {255.*cm, 270.*cm, 1100.*cm, 1200.*cm};
  ...
  G4double friPosZ[2] = {300.*cm, 310.*cm};  //replaces downstream pizzas from conceptual design - Position of FRI 
  //G4double friPosZ[2] = {300.*cm, 310.*cm};  //OLD - too close to dipole
