...
G4VSolid* apert = new G4Tubs("Aperture", 0., gdTube_outerRad,    // ← gdTube_outerRad undefined after commenting out GuideTube
  ...
  */
  ...
  //G4double trkrPosZ[4] = {370.*cm, 380.*cm, 680.*cm, 690.*cm}; // OLD - too close to dipole
  G4double trkrPosZ[4] = {370.*cm, 380.*cm, 680.*cm, 690.*cm}; // CORRECTED - proper positions
  ...
  //G4double pizzaPosZ[4] = {345.*cm, 360.*cm, 1100.*cm, 1200.*cm};
  G4double pizzaPosZ[4] = {345.*cm, 360.*cm, 1100.*cm, 1200.*cm};
  ...
  G4double friPosZ[2] = {390.*cm, 400.*cm};  //replaces downstream pizzas from conceptual design - Position of FRI 
  //G4double friPosZ[2] = {390.*cm, 400.*cm};  //OLD - too close to dipole
