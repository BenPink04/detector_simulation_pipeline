...
G4VSolid* apert = new G4Tubs("Aperture", 0., gdTube_outerRad,    // ← gdTube_outerRad undefined after commenting out GuideTube
  ...
  */
  ...
  //G4double trkrPosZ[4] = {320.*cm, 330.*cm, 360.*cm, 370.*cm}; // OLD - too close to dipole
  G4double trkrPosZ[4] = {320.*cm, 330.*cm, 360.*cm, 370.*cm}; // CORRECTED - proper positions
  ...
  //G4double pizzaPosZ[4] = {295.*cm, 310.*cm, 1100.*cm, 1200.*cm};
  G4double pizzaPosZ[4] = {295.*cm, 310.*cm, 1100.*cm, 1200.*cm};
  ...
  G4double friPosZ[2] = {340.*cm, 350.*cm};  //replaces downstream pizzas from conceptual design - Position of FRI 
  //G4double friPosZ[2] = {340.*cm, 350.*cm};  //OLD - too close to dipole
